-- Create members table with comprehensive profile data (enum already exists)
...
-- Create penalties table that references members
...
CREATE POLICY "Only authenticated users can insert penalties" 
ON public.penalties 
FOR INSERT 
TO authenticated
WITH CHECK (true);

CREATE POLICY "Only authenticated users can update penalties" 
ON public.penalties 
FOR UPDATE 
TO authenticated
USING (true);

CREATE POLICY "Only authenticated users can delete penalties" 
ON public.penalties 
FOR DELETE 
TO authenticated
USING (true);
...
CREATE INDEX idx_penalties_member_id ON public.penalties(member_id);
