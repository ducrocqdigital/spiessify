...
-- First, drop all dependent functions and policies
DROP POLICY IF EXISTS "Only authorized users can manage penalties" ON penalties;
DROP FUNCTION IF EXISTS public.get_members_with_public_stats();
DROP FUNCTION IF EXISTS public.has_role(_user_id uuid, _role text);
DROP FUNCTION IF EXISTS public.get_user_profile(_user_id uuid);
DROP FUNCTION IF EXISTS public.user_can_manage_members();
