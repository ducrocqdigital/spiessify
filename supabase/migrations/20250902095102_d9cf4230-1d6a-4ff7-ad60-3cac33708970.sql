...
-- First, drop ALL policies and functions that depend on the enum
...
DROP FUNCTION IF EXISTS public.get_members_with_public_stats() CASCADE;
DROP FUNCTION IF EXISTS public.has_role(_user_id uuid, _role text) CASCADE;
DROP FUNCTION IF EXISTS public.get_user_profile(_user_id uuid) CASCADE;
DROP FUNCTION IF EXISTS public.user_can_manage_members() CASCADE;
...
-- Recreate all the functions with updated ranks
...
-- Recreate policies with updated ranks
CREATE POLICY "Authenticated users can manage members" 
ON members FOR ALL 
USING (user_can_manage_members())
WITH CHECK (user_can_manage_members());
